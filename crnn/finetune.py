...
parser.add_argument('--datasetname', required=True, help='name of the test dataset')
#parser.add_argument('--valRoot1', required=True, help='path to second dataset')
...
def check_data(data_loader, name='sample'):
        ...
        print(cpu_texts)
        cv2.imwrite('/home2/rafaelgetto/sanjana_model/code/3/crnn_new/new_files/outputs/fine_tune_outputs/{}.jpg'.format(name), out)
...
#test_dataset1 = dataset.lmdbDataset(root=opt.valRoot1, transform=dataset.resizeNormalize((100,32)))
train_loader = torch.utils.data.DataLoader(
    train_dataset, batch_size=opt.batchSize,
...
# lexiconlist_filename = '/ssd_scratch/cvit/sanjana/crnn_new/{0}lexicon.txt'.format(opt.lan) 
p = open(opt.charlist,'r').readlines()
...
if opt.pretrained != '':
    ...
    # print("Before", checkpoint.keys())
    for key in list(checkpoint.keys()):
        if 'module.' in key:
            checkpoint[key.replace('module.', '')] = checkpoint[key]
            del checkpoint[key]
    print("After", checkpoint.keys())
...
#opt.nepoch = 25
output_file = open(os.path.join(opt.expr_dir, f"{opt.lan}_{opt.datasetname}_finetune_metric_logs.txt"), "w")

#writer = SummaryWriter('/home/sanjana.g/runs/exp_hin_preguj_starnet.pth')
print(opt.nepoch, len(train_loader))
    ...
    while i < len(train_loader):
        ...
        if i % opt.displayInterval == 0:
            print('[%d/%d][%d/%d] Loss: %f' %
                  (epoch, opt.nepoch, i, len(train_loader), loss_avg.val()), file = output_file)
            loss_avg.reset()

        if i % opt.valInterval == 0:
            print("Val Dataset - {0}".format(opt.valRoot))
            lossval, acc, crr = val(crnn, test_dataset, criterion)
            print('Test loss: %f, accuracy: %f, crr: %f' % (lossval, acc, crr), file = output_file)

            #print("Val Dataset - {0}".format(opt.valRoot1))
            #_ = val(crnn, test_dataset1, criterion)
            # pdb.set_trace()	
            ...
            if is_best:
                best_acc = acc
                filename = '{0}/best_{1}_{2}_finetune.pth'.format(opt.expr_dir, opt.lan, opt.datasetname)
